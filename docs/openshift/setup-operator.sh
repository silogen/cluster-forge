...
#
# Companion to install.sh, which installs everything in a single run. This
# script is built up app by app so each one can be applied and verified on its
# own; install.sh is kept unchanged for comparison.
...
# AIMClusterRuntimeConfig "default" with different halves of its spec -- and install.sh
...
# one configured, which is how the routing setting install.sh deliberately sets on
...
#
#     CF_PROXY_URL=http://proxy.corp:3128 ./setup-operator.sh
#
...
# they live in extra/ next to this script. Resolved exactly the way install.sh
...
# cluster-auth shim's Python, which install.sh fetches the same way from
...
# ingresses.config.openshift.io/cluster (the same source install.sh reads).
...
#
#     KUBECONFIG=docs/openshift/kube.yaml ./setup-operator.sh
#
...
# missing page.
#
# https always, unlike install.sh, which also has a localhost shape on plain http with
...
# ============================================================================
# Where the AMD GPU operator runs, discovered the same way install.sh does it, by
...
#
# install.sh takes the literal "default" with an env override, which is right on the
...
# has no cluster to ask. Rendered and applied, as both this script and install.sh do
...
# install.sh uses, because the two must agree about what they mean.
#
...
#
# The defaults are placeholders, as install.sh's are. Set them for any deployment
# that is not a throwaway:
#
#     MINIO_API_ACCESS_KEY=... MINIO_API_SECRET_KEY=... ./setup-operator.sh
#
...
# install.sh's and describe a MinIO container on a Rancher Desktop host, so they are
...
# exist already. The host default is install.sh's and is a Docker Desktop alias, so it
...
# default as install.sh and as values.yaml; override it on any cluster that will
# serve real traffic.
...
# the well-known name. Same fallback install.sh uses.
...
# objects install.sh creates with an inline heredoc: an OpenShift-only RBAC
...
#
# Applied before verify:, matching install.sh, which creates the kyverno RBAC
...
render_extra_objects() {
  ...
  # The objects that need it are the credential Secrets install.sh assembles with
...
# install.sh does with `kubectl create configmap --from-file`.
#
...
# file stays where it is and is read from there, so this script and install.sh run the
...
#
# Split out of install_extra so uninstall-operator.sh reads the same file through the
...
install_extra() {
  ...
  # Piped through server-side apply when substituting, which is what install.sh
...
#     which is why install.sh renders that chart with --no-hooks.
#
...
# exactly those objects. install.sh handles it by applying the whole chart three
...
#
# install.sh works around this by switching the context around the apply, which is
...
# half a list.
#
# For the text edits install.sh makes to a manifest on its way to the cluster.
...
# install.sh's own seds are, so the ones that need anchors and a backreference
...
# stale cache. install.sh clears it after the external-secrets CRDs for exactly
...
helm_parameters() {
  ...
  for (( i = 0; i < n; i++ )); do
    # when: VAR=value — pass this parameter only in that deployment shape. install.sh
...
#
# Split out of install_core because uninstall-operator.sh has to find the same chart to
...
# by one function. uninstall-operator.sh renders each step exactly as it was installed and
...
# came from.
#
# Split out for uninstall-operator.sh, which needs the same chart at the same version to
...
fetch_external_chart() {
  ...
  # The same override install.sh offers, for a chart that has been published which no
...
#
# For the steps install.sh writes as a run of `kubectl create secret --from-literal`
...
step_local_path_storage() {
  ...
  # "default" as their storageClassName, and install.sh passes it explicitly to
...
step_copy_objects() {
  ...
  for (( i = 0; i < total; i++ )); do
    ...
    # applied as a new object. ownerReferences goes too, which install.sh does not
...
PROBE
  ...
  # Fatal, unlike install.sh, which prints the same finding and carries on. A
...
#
# CF_LIB_ONLY=true stops here: uninstall-operator.sh sources this file for its
...
install_all() {
  ...
  for app in "${CF_APPS[@]}"; do
    ...
    # external database, an external object store), and install.sh expresses that
    ...
    # this field", which on a cluster carrying leftovers from ArgoCD, helm, install.sh
