...
# Auto-detect the node's internal IP if no explicit range was given. On cloud VMs
# (Oracle, AWS, GCP, Azure) the public IP is NAT'd to the node's private IP, so using
# the private IP here makes the LoadBalancer reachable via the public IP.
if [[ -z "${METALLB_IP_RANGE}" ]]; then
  NODE_IP=$(kubectl get nodes -o jsonpath='{.items[0].status.addresses[?(@.type=="InternalIP")].address}')
  METALLB_IP_RANGE="${NODE_IP}-${NODE_IP}"
  echo "ℹ️  Auto-detected MetalLB IP range from node internal IP: ${METALLB_IP_RANGE}"
fi
...
# ENVOY GATEWAY
# ============================================================================
# Envoy Gateway is the Gateway API implementation (replaces kgateway).
# The Gateway API + envoy CRDs were installed early (see GATEWAY API CRDs above);
# here we install the controller and then the gateway configuration
# (GatewayClass, Gateway, traffic/security policies, services) for the domain.
echo "📦 Installing Envoy Gateway..."
kubectl create namespace envoy-gateway-system --dry-run=client -o yaml | kubectl apply -f -
helm template envoy-gateway ${SOURCES_DIR}/envoy-gateway/v1.7.1 \
  --namespace envoy-gateway-system \
  | kubectl apply --server-side --force-conflicts -f -

# Wait for the Envoy Gateway controller to be ready
echo "⏳ Waiting for Envoy Gateway controller to be ready..."
kubectl wait --for=condition=available --timeout=180s deployment/envoy-gateway -n envoy-gateway-system

# The EnvoyProxy config sets nodeSelector cluster-bloom/first-node=true so the
# Envoy data-plane pod schedules on the designated node. Label all nodes that
# are missing it to ensure the Envoy proxy can always schedule.
for node in $(kubectl get nodes -o jsonpath='{.items[*].metadata.name}'); do
  if ! kubectl get node "${node}" --show-labels 2>/dev/null | grep -q "cluster-bloom/first-node=true"; then
    echo "  Labelling node ${node} with cluster-bloom/first-node=true"
    kubectl label node "${node}" cluster-bloom/first-node=true --overwrite
  fi
done

# Apply Envoy Gateway configuration: GatewayClass, Gateway, traffic/security
# policies and the gateway Service. The chart is templated with the cluster
# DOMAIN so listeners and routing match the deployment hostname.
# The ext-auth ReferenceGrant is created in the cluster-auth namespace, so it
# must exist before applying the config (the cluster-auth shim is installed later).
...
echo "📦 Applying Envoy Gateway configuration (domain: ${DOMAIN})..."
