...
2. **AIRM source code:**
   ```bash
   cd ~/projects/llm-studio-core/services/airm
   ```
...
```bash
cd ~/projects/cluster-forge
./scripts/local-dev-portforward.sh
```
...
- **Automatically generate `.env` files** with credentials from the cluster:
  - `~/projects/llm-studio-core/services/airm/api/.env`
  - `~/projects/llm-studio-core/services/airm/ui/.env.local`
...
```bash
cd ~/projects/llm-studio-core/services/airm/api
...
```bash
cd ~/projects/llm-studio-core/services/airm/ui
