...
1. Set environment variables:
   ...
   export MINIO_API_ACCESS_KEY=<access-key>
   export MINIO_API_SECRET_KEY=<secret-key>
...
- Skips creating the in-cluster Tenant `default-user` Secret (no Tenant to
  bootstrap)
- Creates `minio-credentials` Secrets in the `aiwb` and `workbench`
  namespaces from `${MINIO_API_ACCESS_KEY}` / `${MINIO_API_SECRET_KEY}`
