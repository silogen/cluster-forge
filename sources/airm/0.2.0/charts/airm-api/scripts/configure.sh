...
CLUSTER_BASE_URL="https://workspaces.${NEW_DOMAIN_NAME}/"
...
if [ -z "$AIRM_API_URL" ]; then
    echo "ERROR: AIRM_API_URL environment variable is not set."
    exit 1
fi

if [ -z "$KEYCLOAK_URL" ]; then
    echo "ERROR: KEYCLOAK_URL environment variable is not set."
    exit 1
fi
...
if [ -z "$KEYCLOAK_CLIENT_ID" ]; then
    echo "ERROR: KEYCLOAK_CLIENT_ID environment variable is not set."
    exit 1
fi

if [ -z "$KEYCLOAK_ADMIN_CLIENT_ID" ]; then
    echo "ERROR: KEYCLOAK_ADMIN_CLIENT_ID environment variable is not set."
    exit 1
fi

if [ -z "$KEYCLOAK_ADMIN_CLIENT_SECRET" ]; then
    echo "ERROR: KEYCLOAK_ADMIN_CLIENT_SECRET environment variable is not set."
    exit 1
fi
...
function refresh_token() {
    TOKEN=$(curl -s -d "client_id=${KEYCLOAK_CLIENT_ID}" -d "username=${USER_EMAIL}" -d 'password=password' -d 'grant_type=password' -d "client_secret=${KEYCLOAK_CLIENT_SECRET}" "${KEYCLOAK_URL}/realms/airm/protocol/openid-connect/token" | jq -r '.access_token')
...
function add_user_to_org() {
    ...
    USER_EXISTS=$(curl -s -X GET "${AIRM_API_URL}/v1/users" -H 'accept: application/json' -H "Authorization: Bearer ${TOKEN}" -H 'Content-Type: application/json' | jq -r --arg email "$USER_EMAIL" '.users? // [] | .[] | select(.email==$email) | .email')
...
function request_password_reset() {
    ADMIN_TOKEN=$(curl -X POST "${KEYCLOAK_URL}/realms/airm/protocol/openid-connect/token" \
    ...
    USER=$(curl -X GET "${KEYCLOAK_URL}/admin/realms/airm/users?email=${USER_EMAIL}&exact=true" \
    ...
    UPDATE_USER_RESP=$(curl -w "%{http_code}" -o /dev/null -s -X PUT \
        "${KEYCLOAK_URL}/admin/realms/airm/users/${USER_ID}" \
