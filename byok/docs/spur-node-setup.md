...
The node needs no helm, kubectl, yq, jq or git: the `spur-aims` binary
...
`spur aims ...` works when the `spur` build holds the plugin mechanism.
`spur-aims ...` works with any build.
...
- `allow_admin_kubeconfig = true` lets `spur-aims` ask Spur for the admin
...
```bash
spur aims list
spur aims install --smoke-test          # the default profile
spur aims status
```
...
Gate: every capability of the profile reads `yes` in `spur aims status`, the
...
```bash
spur aims uninstall --yes         # every profile; --keep-data keeps the PVCs and CRDs
...
`uninstall` leaves the namespace `aims-system` of the install record, and the
