...
if ! kubectl cluster-info &> /dev/null; then
    ...
    echo "   kind create cluster --name cluster-forge-local --config scripts/utils/kind-cluster-config.yaml"
    exit 1
fi

# ─── Inject corporate CA certificates into Kind node ─────────────────────
# Behind a TLS-intercepting proxy, containerd inside Kind can't verify
# registry certificates. Copy the CA certs into the node and restart containerd.
inject_kind_certs() {
    local cert_search_paths=(
        "${SCRIPT_DIR}/certs"
        "${HOME}/dev_helpers/certs"
        "/usr/local/share/ca-certificates"
    )

    local cert_dir=""
    for p in "${cert_search_paths[@]}"; do
        if [ -f "${p}/AMD_ROOT.crt" ]; then
            cert_dir="${p}"
            break
        fi
    done
    ...
    local node="cluster-forge-local-control-plane"
    local node_cert_dir="/usr/local/share/ca-certificates"

    # Skip if certs already installed
    if docker exec "${node}" test -f "${node_cert_dir}/AMD_ROOT.crt" 2>/dev/null; then
        echo "✅ AMD certificates already installed in Kind node"
        return
    fi

    echo "🔐 Injecting AMD CA certificates into Kind node..."
    for cert in AMD_ROOT.crt AMD_ISSUER.crt; do
        if [ -f "${cert_dir}/${cert}" ]; then
            docker cp "${cert_dir}/${cert}" "${node}:${node_cert_dir}/"
        fi
    done
    docker exec "${node}" update-ca-certificates
    docker exec "${node}" systemctl restart containerd
    echo "   ✅ Certificates installed and containerd restarted"
}
...
# Kind maps NodePorts to host ports (see scripts/utils/kind-cluster-config.yaml).
